...
class DoctorSignInFragment : Fragment() {
  private var mListener: CFMedicoRegistro? = null
  ...
  private fun validar(): Boolean {
    ...
    cedulaProf = (ac.findViewById<View>(R.id.editCedulaProf) as EditText).text.toString()
    ...
    return if (fechaNac.isEmpty()) {
      verifEditText(R.string.falta_fecha_nac, R.id.dtpFechaNac)
    } else true
  ...
  fun registrarse() {
    if (validar()) {
      ...
      try {
        ...
        val resultado = JSONPostRequest().execute(URL_REGISTRO, request.toString()).get()
  ...
  override fun onAttach(activity: Activity) {
    ...
    if (activity is CFMedicoRegistro) {
      mListener = activity
    } else {
