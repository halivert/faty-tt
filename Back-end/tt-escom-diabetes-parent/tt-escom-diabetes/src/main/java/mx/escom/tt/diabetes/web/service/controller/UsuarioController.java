...
public class UsuarioController {
	...
	/**
	 * Proposito : Recuperar la informacion de un usuario por medio de su
	 * Identificador.
	 * 
	 * @author Edgar, ESCOM
	...
	@RequestMapping(value = "/usuarios/dietas/pdf", method = RequestMethod.POST, produces = "application/pdf")
	public ResponseEntity<?> generarDietaPDF(@RequestBody DietaReportePDFVo dietaReportePDFVo) {
		log.debug("Inicio - Controller");
		...
		try {
			...
			response = usuarioFacade.generarDietaPDF(dietaReportePDFVo);
			//result = new ResponseEntity<RespuestaVo>(respuestaVo, HttpStatus.OK);
			...
			if(response != null) {
	            ...
	            //respHeaders.setContentDispositionFormData(filename, filename);

				result = new ResponseEntity<byte[]>(response, respHeaders, HttpStatus.OK);
